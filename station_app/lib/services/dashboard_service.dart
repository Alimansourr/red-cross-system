...
class CalendarMarker {
  final DateTime date;
  final String title;
  final String type; // event | off_day
...
class DashboardService {
  ...
  Future<Map<String, List<CalendarMarker>>> fetchCalendarMarkersForMonth(
    ...
    // OFF DAYS
    try {
      final offDaysSnapshot = await _db.collection('off_days').get();

      for (final doc in offDaysSnapshot.docs) {
        final data = doc.data();
        ...
        markers[key]!.add(
          CalendarMarker(
            date: normalizedDate,
            title: (data['title'] ?? 'Off Day').toString(),
            type: 'off_day',
          ),
        );
      }
    } catch (_) {
      // off_days collection does not exist yet
    }
